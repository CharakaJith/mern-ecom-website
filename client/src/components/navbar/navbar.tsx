        ...
        <div className="hidden md:flex items-center space-x-6">
          {NAV_LINKS.map(({ href, label }) => (
            <NavLink
              ...
              to={href}
              className={({ isActive }) =>
                `relative text-sm md:text-lg transition-colors duration-300 group ${isActive ? 'text-blue-800' : 'text-gray-900 hover:text-blue-800'}`
              }
            >
              {({ isActive }) => (
                <>
            ...
            {NAV_LINKS.map(({ href, label, icon: Icon }) => (
              <NavLink
                ...
                className={({ isActive }) => {
                  ...
                  return `flex items-center gap-2 hover:text-gray-700 ${colorClass} ${isActive ? 'underline underline-offset-5 font-semibold' : ''}`;
                }}
              >
